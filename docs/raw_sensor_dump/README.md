...
Permanent copy of the first 300 IMU and odom messages from an archived bag, for reference and plotting.

## Files

| File | Source topic | ~Duration | Columns |
|------|--------------|----------|---------|
| `imu_raw_first_300.csv` | IMU topic from bag | ~1.5 s | stamp_sec, gyro_x, gyro_y, gyro_z, accel_x, accel_y, accel_z |
| `imu_raw_first_3000.csv` | IMU topic from bag | ~15 s | same (for dead-reckon plot aligned with odom) |
| `odom_raw_first_300.csv` | `/odom` | ~15 s | stamp_sec, x, y, z, qx, qy, qz, qw, vx, vy, vz, wx, wy, wz |

## How generated

```bash
.venv/bin/python tools/dump_raw_imu_odom.py <bag_path> --max-imu 300 --max-odom 300 --out-dir docs/raw_sensor_dump
```
...
- IMU: gyro in rad/s, accel in m/s² (sensor frame).
