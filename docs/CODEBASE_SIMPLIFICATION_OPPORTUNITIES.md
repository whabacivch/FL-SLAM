...
**How:** Remove unused args/imports; keep one canonical path per operator.
...
**Where:** `pipeline.py`, `backend_node.py`, `primitive_map.py`, `sinkhorn_ot.py`.
...
- **Pipeline diagnostics block:** Full ScanDiagnostics has been removed in favor of minimal tape + cert summaries, eliminating the large `L_total_np` host sync. Keep future diagnostics in JAX where possible to avoid reintroducing device→host bottlenecks.
...
**Where:** `primitive_map.py` (`primitive_map_merge_reduce`), `sinkhorn_ot.py` (cost matrix build), `backend_node.py` (time_offset), `pipeline.py` (insert event_log, diagnostics, IMU jerk).
...
- **sinkhorn_ot.py — cost matrix:** `for i in range(N): for j in range(M): C[i,j] = d_pos + beta*d_dir`. With N,M ≈ 256+512 = 768, that’s ~590k Python iterations and NumPy indexing. Cost is pure NumPy (w2_sq_2d, _hellinger2_vmf); no JAX in the inner loop.  
  **How:** Vectorize: compute `C` as (N,M) with broadcasting or one call to a vectorized `w2_sq_2d` over (N,M) (e.g. `mu_cam[:, None, :]`, `mu_lidar[None, :, :]`) and similarly for direction term. Then C is one (N,M) array. Reduces 590k iterations to a few array ops.
...
- **pipeline.py — IMU jerk:** `for i in range(len(imu_accel_np) - 1):` with NumPy arrays; length is bounded by MAX_IMU_PREINT_LEN (512). Pure NumPy; no JAX. Could be vectorized with NumPy (diff, norm, divide by dt) in one go to avoid Python loop.
...
- **Diagnostics (pipeline):** We sync L_evidence and h_evidence to NumPy, then compute logdet, trace, eigvals, etc. on host. **Eliminate:** Compute all diagnostic scalars in JAX (jnp.linalg.eigvalsh, jnp.trace, jnp.sum(jnp.log(...)), etc.); then sync only the scalar results (e.g. one struct or small array). Removes one large (506-float) sync and keeps work on device.
...
| P0 | **Diagnostics scalar bundle** | `pipeline.py` | Sync L_evidence, h_evidence to host; NumPy eigvalsh, trace, logdet, etc. | One JIT: inputs (L_evidence, h_evidence, pose, imu_accel, …) → outputs (logdet_L, trace_L, L_dt, trace_L_ex, eig_min/max, jerk_mean/max, distance_pose, …) as a single struct or flat array. One sync of ~30 floats. |
...
| P1 | **Sinkhorn cost matrix** | `sinkhorn_ot.py` | Python double loop over (N,M); NumPy w2_sq_2d, _hellinger2_vmf | JAX: vectorized (N,M) cost C = w2_sq_2d_batch(mu_cam, Sigma_cam, mu_lidar, Sigma_lidar) + beta * hellinger2_vmf_batch(...). Same file or JAX module; keep Sinkhorn iterations as NumPy or move to JAX (see consolidation). |
...
| P2 | **IMU jerk diagnostics** | `pipeline.py` | Python loop over imu_accel_np; NumPy norm, mean, max | JAX: diff along time, norm, then jnp.mean / jnp.max. Fold into diagnostics kernel (P0) or tiny JIT. |
...
| **Association: cost + Sinkhorn** | `sinkhorn_ot.py`: build_cost_matrix (NumPy loop) → sinkhorn_unbalanced_fixed_k (NumPy loop). Primitive_association calls these then uses π in Python. |  JAX cost matrix (vectorized) + JAX Sinkhorn (lax.fori_loop K iterations); single JIT (cost + Sinkhorn) → π.  |
...
4. **Phase 4 (association):** Vectorize Sinkhorn cost in JAX; optionally move Sinkhorn iterations to JAX (lax.fori_loop) so cost + π live on device until pipeline consumes π.
5. **Phase 5 (cleanup):** LiDAR transform in JAX ; tempering scalar JIT if not already in Phase 3; IMU jerk in diagnostics kernel. Ensure we remove all legacy code and simplify!
...
**Suggested order (additions):** Follow **§7.4 phased order**: (1) Merge_reduce full JIT; (2) Diagnostics kernel JIT; (3) Evidence + tempering + excitation consolidation; (4) Sinkhorn cost (and optionally iterations) in JAX; (5) LiDAR transform / tempering / jerk cleanup. Do 4 (time_offset vectorization) as a quick win early.
