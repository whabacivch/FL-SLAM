...
**Where:** `pipeline.py`, `backend_node.py`, `primitive_map.py`.
...
**How:** (1) Diagnostics: compute logdet, trace, L_dt, trace_L_ex, eigvals in JAX; sync only the resulting scalars (one small array or a struct). (2) Merge_reduce: eliminate the Python loop over `order` (see item 5); for the apply loop over `selected_pairs`, batch the updates in JAX (e.g. vmap over pairs, single scatter) so no per-pair float() or sync.
...
**Where:** `pipeline.py` (insert event_log).

**What:** Large Python loops or loops that force repeated JAX→host sync.

**Findings:**

**Completed:** `primitive_map_merge_reduce` is now JAX-only (pair selection + merge application); no Python loops remain.
...
| **Association: cost + Sinkhorn** | **Runtime path:** `primitive_association.py` already uses JAX cost (`_compute_sparse_cost_matrix_jax`) and JAX Sinkhorn (`_sinkhorn_unbalanced_fixed_k_jax`). | No change needed for runtime. |
...
1. **Phase 1 (diagnostics):** Diagnostics kernel JIT: all L/h/pose/IMU-derived scalars in one JIT, one sync. Drop current “sync L,h then NumPy eigvalsh/trace” path.
2. **Phase 2 (evidence fusion):** Consolidate evidence + tempering + excitation into one JIT; single sync for (L_fused, h_fused, alpha, beta, s_dt, s_ex); build certs from that.
3. **Phase 3 (cleanup):** Tempering scalar JIT if not already in Phase 2; IMU jerk in diagnostics kernel (or remove unused tape fields). Ensure we remove all legacy code and simplify!
...
| **4** | **Callbacks / queues** | **Medium** | **Low** | **Yes** |
| **5** | **Massive loops (jerk)** | **Medium** | **Low** | **Yes (§1.5, §12.4)** |
| **6** | **JAX→Python→JAX elimination** | **High (diagnostics)** | **Med** | **Yes (§12.5, §12.8)** |
| **7** | **JAX migration + kernel consolidation** | **High (perf, maintainability)** | **Phased (see §7)** | **Yes (§12)** |

**Suggested order (additions):** Follow **§7.4 phased order**: (1) Diagnostics kernel JIT; (2) Evidence + tempering + excitation consolidation; (3) Tempering / jerk cleanup.
