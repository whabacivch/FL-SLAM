...
#
# Usage:
#   ./scripts/inspect_rosbag_topics.sh /path/to/rosbag
...
if [[ -n "$SUGGESTED_PC" || -n "$SUGGESTED_ODOM" ]]; then
    ...
    echo ""
    echo "  POINTCLOUD_TOPIC=${SUGGESTED_PC:-/camera/depth/points} \\"
    echo "  ODOM_TOPIC=${SUGGESTED_ODOM:-/odom} \\"
    echo "  ./scripts/test-3d-integration.sh"
    echo ""
    echo "Or with ros2 launch:"
    echo ""
