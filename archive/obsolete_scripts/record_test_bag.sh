...
WORLD="${WORLD:-turtlebot3_world}"  # Gazebo world: turtlebot3_world, turtlebot3_house, turtlebot3_empty_world
...
echo "=========================================="
echo "Step 1: Launching Gazebo (Headless)"
echo "=========================================="
echo ""

# Check if xvfb is installed
if ! command -v xvfb-run &> /dev/null; then
    echo "ERROR: xvfb not found. Install with: sudo apt install xvfb" >&2
    exit 1
fi
...
# Force Qt to use offscreen rendering (no GUI window)
export QT_QPA_PLATFORM=offscreen

# Software rendering for headless operation
export LIBGL_ALWAYS_SOFTWARE=1

# Launch Gazebo in headless mode with xvfb
...
echo ""
echo "Available worlds for recording:"
echo "  - turtlebot3_world (default) - obstacles, walls, boxes"
echo "  - turtlebot3_house - indoor house with rooms (best for loops)"
echo "  - turtlebot3_empty_world - empty plane (not recommended)"
echo ""
echo "To use a different world:"
echo "  WORLD=turtlebot3_house ./scripts/record_test_bag.sh"
echo ""
else
