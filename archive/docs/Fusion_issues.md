...
- IMU measurements **are ingested** and fused via `ImuVMFGravityEvidence` and `ImuGyroRotationEvidence`.
- Deskew uses IMU preintegration (constant-twist), and the fused IMU evidence updates the belief (L/h).
- The spec’s UT moment-match step is **not** used; we use closed-form Laplace evidence instead.

So the IMU *hook* is now active end-to-end, but the **specific UT-based approximation from the spec is not the path taken**.
...
- This doc does **not** claim the fused pipeline is “correct SLAM” yet.
- It only documents current wiring + evidence paths and where they diverge from the spec.
