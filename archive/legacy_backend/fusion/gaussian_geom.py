...
def gaussian_frobenius_correction(delta: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    stats = {
        "delta_norm": 0.0,
        "input_stats": {"delta": vec_stats(d)},
        "output_stats": {"delta_corr": vec_stats(delta_corr)},
    }
    ...
    **Information-Geometric Interpretation:**
    ...
    if n_dim >= 6:
        ...
        return delta_out, {
            ...
            "input_stats": {"delta": vec_stats(d)},
            "output_stats": {"delta_corr": vec_stats(delta_out)},
        }
...
def imu_tangent_frobenius_correction(
    ...
    **Tangent-Space Correct Pattern:**
    ...
    if rot_norm < 1e-10:
        ...
        return (
            ...
            dr.copy(), 
            {
                ...
                # OpReport fields
                "delta_norm": 0.0,
                "input_stats": {"delta": vec_stats(delta_full)},
                "output_stats": {"delta_corr": vec_stats(delta_full)},
            }
        )
    ...
    stats = {
        ...
        # OpReport diagnostic fields
        "delta_norm": float(np.linalg.norm(delta_corr_full - delta_full)),
        "input_stats": {"delta": vec_stats(delta_full)},
        "output_stats": {"delta_corr": vec_stats(delta_corr_full)},
    }
