...
COVARIANCE MODEL:
    The covariance uses declared prior scales (from constants):
        - Base uncertainty from MSE (residual variance)
        - Scale ratios defined in constants (no magic numbers)
        - Observability handled by information weight (not covariance)
        - No Jacobians
...
# Theoretical constants are declared in constants.py (no magic numbers)
...
def icp_information_weight(
    ...
    dof_weight = 1.0 / (1.0 + math.exp(-constants.K_SIGMOID * (n_eff - constants.N_MIN_SE3_DOF)))
...
def icp_covariance_tangent(
    ...
    """
    Compute ICP covariance in se(3) tangent space using prior scales.
    
    No Jacobians - uses declared prior covariance scales from constants.
    MSE provides the base uncertainty level.
    Observability (point count) is handled by icp_information_weight separately.
    ...
    if src_transformed.size == 0:
        return np.eye(6, dtype=float) * constants.ICP_COVARIANCE_PRIOR_DEGENERATE
    
    sigma2 = max(mse, constants.WEIGHT_EPSILON)
    
    # Diagonal covariance: MSE * prior scale ratios
    cov = np.diag([
        sigma2 * constants.ICP_COV_TRANS_SCALE,
        sigma2 * constants.ICP_COV_TRANS_SCALE,
        sigma2 * constants.ICP_COV_TRANS_SCALE,
        sigma2 * constants.ICP_COV_ROT_SCALE,
        sigma2 * constants.ICP_COV_ROT_SCALE,
        sigma2 * constants.ICP_COV_ROT_SCALE,
    ])
...
def transport_covariance_to_frame(
    ...
    """
    from fl_slam_poc.common.geometry.se3_numpy import se3_adjoint
